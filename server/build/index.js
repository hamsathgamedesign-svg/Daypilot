...
const createAccountToken = (accountId) => {
    const payload = Buffer.from(JSON.stringify({ accountId, exp: Date.now() + 30 * 24 * 60 * 60 * 1000 })).toString("base64url");
    const signature = crypto_1.default.createHmac("sha256", accountTokenSecret()).update(payload).digest("base64url");
    return `${payload}.${signature}`;
};
const readAccountToken = (token) => {
    const [payload, signature] = token.split(".");
    if (!payload || !signature)
        return null;
    const expected = crypto_1.default.createHmac("sha256", accountTokenSecret()).update(payload).digest("base64url");
    ...
    try {
        ...
        if (!decoded.accountId || !decoded.exp || decoded.exp < Date.now())
            return null;
...
async function start() {
    ...
    app.post("/api/assistant", async (req, res) => {
        ...
        if (!apiKey) {
            res.status(503).json({ error: "OpenAI assistant is not configured on the server" });
            return;
        }
        ...
        try {
            const response = await fetch("https://api.openai.com/v1/chat/completions", {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: "system",
                            content: "You are DayPilot AI, a concise personal productivity assistant. Give practical, friendly advice based only on the provided tasks and reminders. Do not claim to have completed actions. Return plain text with short paragraphs or bullets.",
                        },
                        {
                            role: "user",
                            content: JSON.stringify({ request: prompt, tasks, reminders }),
                        },
                    ],
                }),
            });
            ...
            if (!response.ok) {
                console.error("OpenAI assistant request failed:", response.status, result.error?.message || "Unknown error");
                res.status(502).json({ error: "The AI assistant could not respond" });
                return;
            }
            const answer = result.choices?.[0]?.message?.content?.trim();
            if (!answer) {
                res.status(502).json({ error: "The AI assistant returned an empty response" });
                return;
            }
            res.json({ answer });
        }
