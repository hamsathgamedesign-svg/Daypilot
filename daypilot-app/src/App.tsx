    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/assistant`, {
        ...
        body: JSON.stringify({
          ...
          tasks,
          reminders: [...importantReminders, ...dateReminders],
        }),
      })
      const result = await response.json() as { answer?: string; error?: string }
      if (!response.ok || !result.answer) throw new Error(result.error || 'The AI assistant could not respond')
      setAssistantResponse(result.answer)
    } catch (error) {
      setAssistantError(error instanceof Error ? error.message : 'The AI assistant could not respond')
    } finally {
          ...
          <section className="wide-grid secondary-grid">
            ...
            <div className="panel assistant-panel">
              ...
              <div className="assistant-box">
                <p>“Plan my evening.”</p>
                <button type="button" onClick={askAssistant} disabled={assistantLoading}>
                  {assistantLoading ? 'Thinking...' : assistantOpen ? 'Ask again' : 'Ask assistant'}
                </button>
                {assistantOpen && (
                  <p className="assistant-response">{assistantError || assistantResponse}</p>
                )}
              </div>
            </div>
